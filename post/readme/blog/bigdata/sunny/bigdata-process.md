...
## 引言
```md
整个大数据的生态体系十分庞大，繁多的概念，各类技术还处在一个蓬勃发展的阶段。
希望以我目前的认知范围、 能用简单的语言 来讲述清楚什么是大数据。

本篇主要从大数据的处理流程来讲述什么是大数据。
```
...
### 数据源
```md
首先，要清楚的问题是大数据处理的数据来源。从抽象层面来说，企业系统中能产生数据的地方都可以作为数据的来源。
```
![](pic/datasource.png)
```md
实际常见数据源的是可以区分为几类：
...
2. 关系型数据库
   MySQL、SQLServer、Oracle等。（关系型数据库从数据的存储方式看也是一系列结构化的文件）
3. 缓存类
...
可以看出，数据源的范畴很宽泛，但总体看都带有存储特性（内存或磁盘）。

对从数据源获取的原生数据直接做处理是一个理想的选择，但是很难实现，数据源的异构特性带来的处理复杂度非常高。
所以要从数据特性对数据进行抽象，在处理的复杂度 和 处理的问题范围上做权衡。

从大数据处理角度看，主要有两种划分：
1. 批处理数据（离线数据）和 流数据（实时数据）
   这代表大数据处理的两个方向，从目前来看，离线处理相对成熟，流处理也逐渐成为大数据的热点，同时两者有融合的趋势。
   同时它们也对应了不同的大数据应用场景。

   你可能还不能理解 什么是批处理 和 流处理，在这里我们可以简单的做一个理解:
   批处理数据 具有持久 和 历史 特性，长期保存在HDFS上，随时可以计算和检索，一般情况下数据成线下增长。
   流处理数据 具有实时性，随时读取数据，即时处理，即时获取结果。目前 主流的流处理框架都以 Kafaka 作为数据源。

2. 结构化数据 和 非结构化数据
   这个比较好理解，表数据是结构化的，日志文件 如果定义了格式，就是结构化的，为定义则是非结构化的。
   还有一种 介于 结构化数据 和 非结构化数据之间的半结构化数据，如 XML，可以视场景做结构化处理或非结构化处理。
   这个层面数据特性的区分，也决定了后续的 数据存储、计算、查询方案的不同。

这两个划分方式是交叉的，例如 批处理数据方式，既有结构化的数据处理，也有非结构化的处理，流处理也一样。
